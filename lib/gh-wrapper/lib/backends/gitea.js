 ...
 * On failure, the cache is NOT populated so the next call retries.
...
async function getGiteaVersion() {
  ...
  try {
    ...
    // Only cache on success so a transient failure doesn't permanently disable quirk detection
    _giteaVersion = (data && data.version) ? data.version : 'unknown';
  } catch (err) {
    process.stderr.write(`[gh-wrapper] Could not detect Gitea version, will retry: ${err.message}\n`);
    // Do NOT set _giteaVersion — leave null so next call retries
    return 'unknown';
  }
  return _giteaVersion;
}
...
// gh repo view [owner/repo] [--repo owner/repo] [--json fields] [--jq expr]
async function repoView(flags) {
  const r = parseRepo(flags.repo) || repoFromGit();
  if (!r) throw new Error('--repo is required or run from inside a git repo');

  const repo = await giteaRequest('GET', `${apiBase()}/repos/${r.owner}/${r.repo}`, null, token());
  ...
  const mapped = {
    ...
    url: repo.html_url,
    sshUrl: repo.ssh_url,
    defaultBranchRef: { name: repo.default_branch || 'main' },
    isPrivate: repo.private || false,
    isEmpty: repo.empty || false,
    stargazerCount: repo.stars_count || 0,
...
async function run(argv) {
  ...
  switch (command) {
    ...
    case 'repo':
      switch (subcommand) {
        case 'view': return repoView(flags);
        case 'create': {
