...
// ---------------------------------------------------------------------------
// to_entries / from_entries — both bare and with-parens forms work
// ---------------------------------------------------------------------------

test('to_entries bare (no parens)', () => {
  assert.deepEqual(applyJq('to_entries', { a: 1 }), [{ key: 'a', value: 1 }]);
});

test('to_entries() with parens', () => {
  assert.deepEqual(applyJq('to_entries()', { a: 1 }), [{ key: 'a', value: 1 }]);
});

test('from_entries bare (no parens)', () => {
  assert.deepEqual(applyJq('from_entries', [{ key: 'a', value: 1 }]), { a: 1 });
});

test('from_entries() with parens', () => {
  assert.deepEqual(applyJq('from_entries()', [{ key: 'a', value: 1 }]), { a: 1 });
});

test('to_entries | from_entries round-trips an object', () => {
  const obj = { x: 10, y: 20 };
  assert.deepEqual(applyJq('to_entries | from_entries', obj), obj);
});

// ---------------------------------------------------------------------------
// tojson / fromjson — both bare and with-parens forms work
// ---------------------------------------------------------------------------
...
test('fromjson bare (no parens) decodes string', () => {
  assert.deepEqual(applyJq('fromjson', '{"a":1}'), { a: 1 });
});

test('fromjson() with parens decodes string', () => {
  assert.deepEqual(applyJq('fromjson()', '{"a":1}'), { a: 1 });
});
