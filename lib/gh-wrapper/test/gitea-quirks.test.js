...
test('repo view: calls GET /repos/{owner}/{repo} with correct path', async () => {
  const mock = await startMock([
    {
      method: 'GET', path: '/api/v1/repos/myorg/mybot',
      response: {
        name: 'mybot',
        full_name: 'myorg/mybot',
        description: 'bot repo',
        html_url: 'http://gitea.example.com/myorg/mybot',
        ssh_url: 'git@gitea.example.com:myorg/mybot.git',
        default_branch: 'main',
        private: true,
        empty: false,
        stars_count: 5,
  ...
  try {
    await withEnv(
      ...
      async () => {
        const gitea = freshGitea();
        await suppressOutput(() => gitea.run(['repo', 'view', '--repo', 'myorg/mybot']));
        const req = mock.captured.find(r => r.method === 'GET' && r.url === '/api/v1/repos/myorg/mybot');
        assert.ok(req, 'GET /repos/myorg/mybot was called');
      },
    );
  } finally {
    await mock.stop();
...
test('repo view: resolves full_name as nameWithOwner', async () => {
  // Verify the field mapping is correct by providing a non-trivial full_name
  const mock = await startMock([
    {
      method: 'GET', path: '/api/v1/repos/acme-corp/super-bot',
      response: {
        name: 'super-bot',
        full_name: 'acme-corp/super-bot',
        html_url: 'http://gitea.local/acme-corp/super-bot',
  ...
  try {
    await withEnv(
      ...
      async () => {
        const gitea = freshGitea();
        // Run completes without throwing — field mapping validated below via HTTP capture
        await assert.doesNotReject(
          suppressOutput(() => gitea.run(['repo', 'view', '--repo', 'acme-corp/super-bot'])),
          'repo view should not throw for a valid repo response',
        );
        // Verify the endpoint was called (not a different one)
        const req = mock.captured.find(r => r.method === 'GET');
        assert.ok(req && req.url === '/api/v1/repos/acme-corp/super-bot',
          'called the correct Gitea repos endpoint');
      },
    );
  } finally {
    await mock.stop();
...
test('repo view: unknown subcommand exits with error', async () => {
  const mock = await startMock([]);
  try {
    await withEnv(
      { GITEA_URL: `http://127.0.0.1:${mock.port}`, GITEA_TOKEN: 'tok', GITEA_QUIRKS: '1.24' },
      async () => {
        ...
        try {
          await gitea.run(['repo', 'clone']);
        } catch { /* expected */ }
        process.exit = origExit;
        assert.ok(exited, 'process.exit called for unknown repo subcommand');
      },
    );
  } finally {
    await mock.stop();
