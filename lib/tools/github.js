...
const { getGhToken, getApiBaseUrl } = require('../gh-wrapper/lib/config.js');
...
async function githubApi(endpoint, options = {}) {
  ...
  const isGithub = baseUrl.includes('api.github.com');
  const headers = {
...
async function triggerWorkflowDispatch(workflowId, ref = 'main', inputs = {}) {
  ...
  const res = await fetch(
    `${baseUrl}/repos/${GH_OWNER}/${GH_REPO}/actions/workflows/${workflowId}/dispatches`,
    {
      method: 'POST',
      headers: {
...
async function listRepositories() {
  ...
  const probes = await Promise.allSettled(repos.map(async (r) => {
    const res = await fetch(`${baseUrl}/repos/${r.full_name}/git/refs`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${getGhToken()}`,
        'Accept': isGithub ? 'application/vnd.github+json' : 'application/json',
