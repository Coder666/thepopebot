...
/**
 * Create a create_repository tool bound to a specific user.
 * Creates a new repository in Gitea/GitHub and links it to a new Code Workspace.
 * @param {object} context
 * @param {string} context.userId - The authenticated user ID
 * @returns {import('@langchain/core/tools').StructuredTool}
 */
function createCreateRepositoryTool({ userId }) {
  return tool(
    async ({ name, description, is_private }) => {
      try {
        ...
        // Create the repository via GitHub/Gitea API
        const repo = await githubApi('/user/repos', {
          ...
          body: JSON.stringify({
            ...
            private: !!is_private,
            auto_init: true, // Needs an initial commit to have a base branch
          }),
        });

        // Give Gitea a moment to finalize the repo creation and default branch
        await new Promise((r) => setTimeout(r, 1500));

        const fullName = repo.full_name;
        let defaultBranch = repo.default_branch;

        // Fallback: Gitea 1.25 sometimes doesn't return default_branch immediately
        if (!defaultBranch) {
          try {
            const repoDetails = await githubApi(`/repos/${fullName}`);
        ...
        // Create the code workspace row in the DB
        const workspace = createCodeWorkspace(userId, {
          repo: fullName,
        ...
        return JSON.stringify({
          ...
          message: `Repository created. [Open Code Workspace](/code/${workspace.id})`,
        });
      } catch (err) {
        console.error('[create_repository] Failed:', err);
        return JSON.stringify({
          success: false,
    ...
    },
    {
      ...
      description:
        'Create a new empty repository in the Git backend (Gitea/GitHub) and initialize a Code Workspace for it. Use this when the user asks to "start a new project", "create a repo", or "build a new app". Returns a link to the new workspace.',
      schema: z.object({
        name: z.string().describe('Repository name (e.g. "bitcoin-bot", short and URL-safe)'),
        description: z.string().optional().describe('Short description of the repository.'),
        is_private: z.boolean().optional().describe('Whether the repository should be private (default: true)'),
      }),
    }
  );
}

export { createJobTool, getJobStatusTool, getSystemTechnicalSpecsTool, getSkillBuildingGuideTool, getSkillDetailsTool, createStartCodingTool, createStartHeadlessCodingTool, createGetRepositoryDetailsTool, createGetBranchFileTool, createDelegateToPersonaTool, createCreateRepositoryTool, searchChatHistoryTool };
